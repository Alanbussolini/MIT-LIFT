...
export function parseCSV(text: string): SurveyRow[] {
  ...
  return dataLines.map((line) => {
    ...
    return {
      ...
      creditBank: fixEncoding(cols[25]?.trim() || ''),
      creditProviders: fixEncoding(cols[26]?.trim() || ''),
      creditFamily: fixEncoding(cols[27]?.trim() || ''),
      creditGovernment: fixEncoding(cols[28]?.trim() || ''),
      creditPrivate: fixEncoding(cols[29]?.trim() || ''),
